    ...
    const findVars = (e: A.Exp, pool: Pool): Pool =>
        A.isAtomicExp(e) ? extendPool(e, pool) :
            A.isProcExp(e) ? extendPool(e, reducePool(findVars, e.body, reducePoolVarDecls(extendPoolVarDecl, e.args, pool))) :
                // 3.3.a - recursivly gets into a list and extends the pool with head and tail list
                A.isLitExp(e) && V.isEmptySExp(e.val) ?
                    extendPool(e, pool) :
                    A.isLitExp(e) && V.isCompoundSExp(e.val) ?
                        ((val: V.CompoundSExp) => extendPool(e, findVars(A.makeLitExp(val.val1), findVars(A.makeLitExp(val.val2), pool))))(e.val) :
        ...
        A.isProcExp(exp) ? Opt.bind(inPool(pool, exp), (left: T.TExp) =>
            ...
            A.isLitExp(exp) ?
                (V.isEmptySExp(exp.val) ? Opt.mapv(inPool(pool, exp), (left: T.TExp) =>
                    ...
                    V.isCompoundSExp(exp.val) ? ((val: V.CompoundSExp) => Opt.bind(inPool(pool, exp), (left: T.TExp) =>
                        Opt.bind(inPool(pool, A.makeLitExp(val.val1)), (headTE: T.TExp) =>
                            Opt.mapv(inPool(pool, A.makeLitExp(val.val2)), (tailTE: T.TExp) =>
