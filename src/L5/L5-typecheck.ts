...
import {
    isAppExp, isBoolExp, isCExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
                                                                        ...
                                                                        (p.op === 'string=?') ? makeOk(makeProcTExp([makeStrTExp(), makeStrTExp()], makeBoolTExp())) :
                                                                            (p.op === 'display') ? makeOk(makeProcTExp([T()], makeVoidTExp())) :
                                                                                (p.op === 'newline') ? makeOk(makeProcTExp([], makeVoidTExp())) :
                                                                                    // 3.1
                                                                                    (p.op === 'cons') ? (() => { const tv = T(); return makeOk(makeProcTExp([tv, makeListTExp(tv)], makeListTExp(tv))); })() :
                                                                                        (p.op === 'car') ? (() => { const tv = T(); return makeOk(makeProcTExp([makeListTExp(tv)], tv)); })() :
                                                                                            (p.op === 'cdr') ? (() => { const tv = T(); return makeOk(makeProcTExp([makeListTExp(tv)], makeListTExp(tv))); })() :
...
export const typeofProgram = (exp: Program, tenv: TEnv): Result<TExp> => {
    ...
    const typeofProgramExps = (exps: List<Exp>, env: TEnv): Result<TExp> => {
        // Fail gracefully if the program is empty
        if (!isNonEmptyList<Exp>(exps)) {
            return makeFailure("Unexpected empty list of expressions in program");
        }

        // Extract head and tail for easier TypeScript type inference
        const head = first(exps);
        const tail = rest(exps);

        // If it's the last expression, return its type
